...
use crate::mygl::{VAO, VBOWithStorage, TextureAtlas};
...
impl Chunk {
    pub fn new(position : [i32;3], generator : &Perlin) -> Self{
        let mut chunk = Chunk {blocks : vec![0;CHUNK_SIZE * CHUNK_SIZE * CHUNK_SIZE], position, vao : VAO::new(), vertex_pos : VBOWithStorage::new(), texture_pos : VBOWithStorage::new()};
        ...
        chunk.vao.attrib_pointer(0, &chunk.vertex_pos.vbo, 3, 0, 0, false);
        chunk.vao.attrib_pointer(1, &chunk.texture_pos.vbo, 2, 0, 0, false);
...
fn add_face(vertex_data: &mut Vec<u8>, texture_data : &mut Vec<f32>, atlas : &TextureAtlas, texture : u32, pos : [u8;3], dir : Direction) {
